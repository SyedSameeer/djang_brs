...
def forgot_password(request):
    if request.method == 'POST':
        # Create a PasswordResetForm instance with POST data
        form = PasswordResetForm(request.POST)
        if form.is_valid():
            # If the form is valid, initiate the password reset process
...
def OTPGeneration(request):
    if request.method == 'POST':
        ...
        comment = "Welcome to Travel Tourism. This is your OTP: "
        comment1 = comment + otp
        send_mail(
            "OTP for Travel tourism",
            comment1,
